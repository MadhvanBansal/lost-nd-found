...
public class LostItemForm {
    ...
    public LostItemForm(MainWindow main) {
        this.main = main;
        panel = new JPanel(new BorderLayout());
        panel.setBackground(Color.WHITE);

        JLabel title = new JLabel("Log a Lost Item", JLabel.CENTER);
        ...
        loc.setBorder(BorderFactory.createTitledBorder("Location Area"));
        ...
        JLabel imgLabel = new JLabel("No Image Select", JLabel.CENTER);
        ...
        JButton submit = createBtn("Submit Report", new Color(52, 152, 219));
        ...
        submit.addActionListener(e -> {
            if (name.getText().trim().isEmpty() || desc.getText().trim().isEmpty()) {
                JOptionPane.showMessageDialog(main.getFrame(), "Item name and description are required.");
                return;
            }
            scr.main.model.Item item = new scr.main.model.Item(name.getText().trim(), desc.getText().trim(), loc.getText().trim(), "Lost", selectedPath[0]);
            scr.main.servies.ItemService.addItem(item);
            JOptionPane.showMessageDialog(main.getFrame(), "Successfully added to the Lost database.");
            
            name.setText(""); desc.setText(""); loc.setText("");
            selectedPath[0] = ""; imgLabel.setIcon(null); imgLabel.setText("No Image Selected");
            main.showDashboard();
        });
    }
